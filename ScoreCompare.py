...
# Combine 'Genre' and 'Tags' for CountVectorizer
...
# CountVectorizer for textual features
count_vectorizer = CountVectorizer(max_features=10000, stop_words='english')
count_matrix = count_vectorizer.fit_transform(netflix_data['Combined_Text'].values.astype('U'))

# Cosine Similarity with CountVectorizer
cosine_sim = cosine_similarity(count_matrix, count_matrix)

# k-Nearest Neighbors (KNN) with Numerical and Textual Features
combined_matrix = cosine_similarity(numerical_features_standardized) + cosine_similarity(count_matrix, count_matrix)
...
def recommend(movie_title):
    ...
    print(f"\nTop 5 recommended movies for '{movie_title}' using Cosine Similarity with CountVectorizer:")
    for i in range(1, 6):
